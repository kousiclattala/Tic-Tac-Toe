...
export class AppComponent {
  ...
  checkIsWinner = () => {
    if (
      ...
      this.winMessage = `${this.itemArray[0]} is won`;
    } else if (
      this.itemArray[3] !== 'empty' &&
      this.itemArray[3] === this.itemArray[4] &&
      this.itemArray[3] === this.itemArray[5]
    ) {
      this.winMessage = `${this.itemArray[3]} is won`;
    } else if (
      this.itemArray[6] !== 'empty' &&
      this.itemArray[6] === this.itemArray[7] &&
      this.itemArray[6] === this.itemArray[8]
    ) {
      this.winMessage = `${this.itemArray[6]} is won`;
    } else if (
      this.itemArray[0] !== 'empty' &&
      ...
      this.winMessage = `${this.itemArray[1]} is won`;
    } else if (
